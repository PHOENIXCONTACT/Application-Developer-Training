...
```
$ moryx new <NAME> --steps <LIST-OF-STEPS> --products <LIST-OF-PRODUCTS> --template-url <URL> --branch <BRANCH-NAME>
```
...
> NOTE:
> For this training, the application developer program (ADP), a custom template exists.
> It is tailored to the scenario and removes some autogenerated code for simplicity and demonstation purposes.
> By providing the CLI with the branch name 'adp' as parameter it is selected.
>
> For realworld applications usually both the 'template-url' and 'branch' are not supplied,
> but for special scenarios the default values can be altered to fit the specific usecase.
