...
```
$ moryx new PencilFactory --steps Assembling --products GraphitePencil --branch pencilla
```

> NOTE:
> For the application developer program (ADP) a custom template is used.
> It removes some autogenerated code for simplicity and marks position where snippets from the ADP are to be inserted.
